...
Namespace SAP_HANA
    Public Class C_TruckOwnerChange : Implements ISAP_HANA
...
#End Region
        ...
        Public Sub Form_Creation_MenuEvent(ByRef pVal As MenuEvent, ByRef BubbleEvent As Boolean) Implements ISAP_HANA.Form_Creation_MenuEvent
            Try
                Dim sFileName As String = AppDomain.CurrentDomain.BaseDirectory & "SRF\TruckOwnerChange.srf"
                Dim sFormName As String = "SAP_UDO_OTOC"
                Dim FormUID As String = LoadXMLFiles(__oApplication, sFileName)
                oForm = __oApplication.Forms.GetForm("SAP_UDO_OTOC", __oApplication.Forms.ActiveForm.TypeCount)
            ...
            Try
                ...
                If pVal.BeforeAction = False Then
                    ...
                    ElseIf pVal.ItemUID = "m1" And pVal.ColUID = "Col_0" And pVal.EventType = SAPbouiCOM.BoEventTypes.et_CHOOSE_FROM_LIST Then
                        Existing_CardCode_OnAfterChooseFromList(FormUID, pVal, BubbleEvent)

                    ElseIf pVal.ItemUID = "m1" And pVal.ColUID = "Col_2" And pVal.EventType = SAPbouiCOM.BoEventTypes.et_CHOOSE_FROM_LIST Then
                        New_CardCode_OnAfterChooseFromList(FormUID, pVal, BubbleEvent)
                ...
                ElseIf pVal.BeforeAction = True Then
                    ...
                    ElseIf pVal.ItemUID = "m1" And pVal.ColUID = "Col_2" And pVal.EventType = SAPbouiCOM.BoEventTypes.et_CHOOSE_FROM_LIST Then
                        Matrix1_NEW_CardCode_OnBeforeChooseFromList(FormUID, pVal, BubbleEvent)
                    End If
        ...
        Private Sub DefulatSetting(ByVal FormUID As String, ByRef BubbleEvent As Boolean)
            Try
                ...
                oForm.Items.Item("Item_27").Specific.value = GenrateDocEntry("DocEntry", "@SAP_OTOC")
                oForm.Items.Item("Item_25").Specific.value = GenrateDocEntry("DocEntry", "@SAP_OTOC")


                Dim m1 As SAPbouiCOM.Matrix = oForm.Items.Item("m1").Specific
                AddRowInMatrix(oForm, "@SAP_TOC1", "m1")
        ...
        Private Sub Branch_OnAfterChooseFromList(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
            Try
                ...
                Dim dbsrc As SAPbouiCOM.DBDataSource = oForm.DataSources.DBDataSources.Item("@SAP_OTOC")
        ...
        Private Sub Existing_CardCode_OnAfterChooseFromList(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
            Try
                BubbleEvent = True
                oForm = __oApplication.Forms.GetForm(pVal.FormTypeEx, pVal.FormTypeCount)
                ...
                Dim dbsrc As SAPbouiCOM.DBDataSource = oForm.DataSources.DBDataSources.Item("@SAP_TOC1")
                Dim m1 As SAPbouiCOM.Matrix = oForm.Items.Item("m1").Specific
                    ...
                    If Not oCFLEvento.SelectedObjects Is Nothing Then
                        Try
                            ...
                            If oDataTable.Rows.Count > 0 Then
                                ...
                                dbsrc.SetValue("U_CTCode", dbsrc.Offset, oDataTable.GetValue("CardCode", 0) & "")
                                dbsrc.SetValue("U_COWName", dbsrc.Offset, oDataTable.GetValue("CardName", 0) & "")
        ...
        Private Sub New_CardCode_OnAfterChooseFromList(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
            Try
                BubbleEvent = True
                oForm = __oApplication.Forms.GetForm(pVal.FormTypeEx, pVal.FormTypeCount)
                    ...
                    If Not oCFLEvento.SelectedObjects Is Nothing Then
                        Try
                            ...
                            If oDataTable.Rows.Count > 0 Then
                                ...
                                dbsrc.SetValue("U_NTCode", dbsrc.Offset, oDataTable.GetValue("CardCode", 0) & "")
                                dbsrc.SetValue("U_NOWName", dbsrc.Offset, oDataTable.GetValue("CardName", 0) & "")
        ...
        Private Sub Matrix1_CardCode_OnBeforeChooseFromList(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
            Try
                ...
                SqlQuery2.Append("Select T0.""U_CardCode"" from ""@SAP_OTM"" T0 Where T0.""U_TRegNo""='" + oForm.Items.Item("Item_29").Specific.Value + "' ")
                ...
                If oRset.RecordCount > 0 Then
                    For i As Integer = 0 To oRset.RecordCount - 1
                        ...
                        oCondition.Operation = SAPbouiCOM.BoConditionOperation.co_EQUAL
                        oCondition.CondVal = oRset.Fields.Item("U_CardCode").Value & ""
                        oCondition.BracketCloseNum = 1
                        oRset.MoveNext()
        ...
        Private Sub Matrix1_NEW_CardCode_OnBeforeChooseFromList(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
            Try
                BubbleEvent = True
                oForm = __oApplication.Forms.GetForm(pVal.FormTypeEx, pVal.FormTypeCount)
